...
const getCompanyEmployees = async (req, res) => {
  try {
    if(req.user) {
      ...
      //if not the company's employee return err
      let employees = company.employees
      const isEmployeeOfCompany = employees.some((employee) =>
        employee.firstName === req.user.firstName && employee.lastName === req.user.lastName
      );
      if (!isEmployeeOfCompany) {
        return res.status(403).json({ message: 'You are not an employee of this company' });
      }

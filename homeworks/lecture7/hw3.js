...
const server = http.createServer((req, res) => {
  ...
  if (method === 'GET') {
    ...
    if (url === '/create-post') {
      ...
      const input = `?title=${encodeURIComponent(formData.title)}&content=${encodeURIComponent(formData.content)}`;

      // Redirect back to home.html with the query string
      res.writeHead(302, { 'Location': `/home.html${input}` });
      res.end();
        //res.end(parsedBody);
      });
    } else {
